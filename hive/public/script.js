...
async function createCampaign() {
  ...
  if (isCreatingCampaign) {
    console.log(
      "Campaign creation already in progress, ignoring duplicate request"
    );
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      addProgressMessage(
        "🤖 AI agents are now collaborating on your campaign",
        "info"
      );
...
function trackPhaseChange(newPhase) {
  if (currentPhase !== newPhase) {
    if (currentPhase && phaseStartTime) {
      ...
      addProgressMessage(
        `⏱️ ${currentPhase} phase completed in ${duration}s`,
        "info",
        `${currentPhase}-timing`
      );
    }
    ...
    if (estimates[newPhase]) {
      addProgressMessage(
        `⏳ ${newPhase} phase starting (est. ${estimates[newPhase]})`,
        "info",
        `${newPhase}-estimate`
      );
    }
...
function simulateProgressFromConversation(campaign) {
  ...
  newMessages.forEach((message, index) => {
    ...
    if (speaker === "Campaign Brief") {
      addProgressMessage(
        `📝 Original campaign brief recorded: "${message.message.substring(
          0,
          50
        )}..."`,
        "info",
      ...
      if (message.message.includes("campaign introduction") || index === 0) {
        addProgressMessage(
          `👔 Campaign initiated - PR Manager setting strategic direction`,
          "phase",
          messageKey
        );
      } else if (
        message.message.includes("research") ||
        message.message.includes("Research")
      ) {
        trackPhaseChange("Research");
        addProgressMessage(
          `🔄 Transitioning to Research & Audience Analysis phase`,
          "info",
          messageKey
        );
      } else if (
        message.message.includes("trending") ||
        message.message.includes("trends")
      ) {
        trackPhaseChange("Trending");
        addProgressMessage(
          `🔄 Transitioning to Trending News Analysis phase`,
          "info",
          messageKey
        );
      } else if (
        ...
        addProgressMessage(
          `🔄 Transitioning to Story Development phase`,
          "info",
          messageKey
        );
      } else if (
        message.message.includes("collaborative") ||
        message.message.includes("final")
      ) {
        trackPhaseChange("Collaborative");
        addProgressMessage(
          `🔄 Transitioning to Collaborative Synthesis phase`,
          "info",
          messageKey
        );
      } else {
        addProgressMessage(
          `👔 PR Manager: ${message.message.substring(0, 60)}...`,
          "phase",
          messageKey
        );
      }
      ...
      trackPhaseChange("Research");
      addProgressMessage(
        `🔍 Research Agent: Analyzing target audience and market insights`,
        "info",
        messageKey
      );
      ...
      addProgressMessage(
        `🧠 Strategic Insight Agent: Converting insights to emotional truths`,
        "info",
        messageKey
      );
      ...
      trackPhaseChange("Trending");
      addProgressMessage(
        `📈 Trending Agent: Analyzing current market trends and cultural moments`,
        "info",
        messageKey
      );
      ...
      trackPhaseChange("Story");
      addProgressMessage(
        `✍️ Story Agent: Developing compelling narratives and headlines`,
        "info",
        messageKey
      );
      ...
      trackPhaseChange("Collaborative");
      addProgressMessage(
        `🤝 Collaborative Phase: All agents synthesizing final strategy`,
        "phase",
        messageKey
      );
    }
    ...
    if (message.deliverable) {
      ...
      addProgressMessage(
        `${speakerIcon} ${deliverableType} deliverable completed by ${speaker}`,
        "success",
        `${messageKey}-deliverable`
      );

      // Show quality metrics if available
      if (message.qualityScore !== undefined) {
        const qualityLevel =
          message.qualityScore > 0 ? "high" : "needs improvement";
        addProgressMessage(
          `📊 Quality assessment: ${qualityLevel} confidence`,
          "warning",
          `${messageKey}-quality`
        );
      }
  ...
  if (
    ...
    !progressMessages.some((m) => m.message.includes("Campaign completed"))
  ) {
    addProgressMessage(
      "🎉 Campaign strategy completed successfully!",
      "success",
      "completion"
    );
    addProgressMessage(
      "💾 Campaign saved and ready for implementation",
      "success",
      "saved"
    );
...
function getDeliverableType(deliverable) {
  if (typeof deliverable === "object") {
    ...
    if (deliverable.theme || deliverable.humanTruth)
      return "Integrated Campaign Plan";
  }
  return "Strategy";
}
...
function addProgressMessage(message, type = "info", key = null) {
  ...
  const existingMessageIndex = progressMessages.findIndex(
    (p) => p.key === messageObj.key
  );
...
function renderProgressPanel() {
  ...
  const previousScrollBottom =
    progressContainer.scrollHeight - progressContainer.scrollTop;
  ...
  const newScrollTop = Math.max(
    0,
    progressContainer.scrollHeight - previousScrollBottom
  );
...
async function updateCampaignStatus() {
  try {
    ...
    // If the refinePrompt (textarea) is active, skip major DOM updates to test if they cause the disappearance
    if (document.getElementById("refinePrompt")) {
      console.log(
        "[Debug] refinePrompt is active, skipping major DOM updates during this poll."
      );
      if (campaign.status === "completed") {
        clearInterval(pollingInterval);
        campaignInProgress = false;
      }
      return; // Exit early after checking for completion
    }
...
function updateConversationFlow(campaign) {
  ...
  const userWasAtBottom =
    conversationMessages.scrollHeight - conversationMessages.clientHeight <=
    conversationMessages.scrollTop + scrollThreshold;
  ...
  conversationMessages.innerHTML = campaign.conversation
    .map((message) => {
      ...
      let messageHtml = `
                    <div class="conversation-message ${speakerClass} ${isProcessing}">
                        <div class="message-speaker">${speakerIcon} ${
        message.speaker
      }:</div>
  ...
  if (typeof deliverable === "object") {
    ...
    // Handle comprehensive final strategy deliverables
      ...
      if (deliverable.storyAngles) {
        ...
        if (typeof deliverable.storyAngles === "string") {
          ...
          if (
            deliverable.storyAngles.storyStrategy &&
            typeof deliverable.storyAngles.storyStrategy === "string"
          ) {
            content += `${deliverable.storyAngles.storyStrategy}\\n\\n`;
          }
          // If it has an 'angles' property (as seen in backend story.angles)
          else if (
            deliverable.storyAngles.angles &&
            typeof deliverable.storyAngles.angles === "string"
          ) {
      ...
      if (deliverable.trends) {
        ...
        if (typeof deliverable.trends === "string") {
          ...
          content +=
            deliverable.trends
              .map((trend) => {
                ...
                if (typeof trend === "object" && trend.trendName)
                  return `- ${trend.trendName}: ${trend.description || ""}`; // Example structure
          ...
          if (
            deliverable.trends.trendsAnalysis &&
            typeof deliverable.trends.trendsAnalysis === "string"
          ) {
      ...
      if (!content) {
        content =
          "Comprehensive campaign strategy has been generated. Please check the deliverables panel for full details.";
      }
    }
    // Fallback for any other object structure
      ...
      if (keys.length > 0) {
        ...
        content =
          "Deliverable content is available but in an unexpected format.";
      }
    }
  ...
  try {
    ...
    if (campaigns.length === 0) {
      campaignsList.innerHTML =
        '<p style="color: #666; text-align: center; padding: 20px;">No campaigns yet. Create your first campaign above!</p>';
      return;
    }
    ...
    campaignsList.innerHTML = sortedCampaigns
      .map(
        (campaign) => `
                    <div class="campaign-item" onclick="loadCampaign('${
                      campaign.id
                    }')">
    ...
    const campaignsList = document.getElementById("campaignsList");
    campaignsList.innerHTML =
      '<p style="color: #dc3545; text-align: center; padding: 20px;">Failed to load campaigns. Please refresh the page.</p>';
  }
}
...
function updateDeliverablesPanel() {
  ...
  if (Object.keys(campaignDeliverables).length === 0) {
    content.innerHTML =
      '<p style="color: #666; text-align: center; margin-top: 50px;">No deliverables available yet.</p>';
    return;
  }
...
function openDeliverableModal(agentName, event) {
  ...
  document.getElementById("modalTitle").textContent =
    deliverable.title || agentName;
  document.getElementById("modalBody").innerHTML = formatMarkdown(
...
async function downloadModalPdf() {
  ...
  const rawTitle =
    campaignDeliverables[currentModalDeliverable].title ||
    currentModalDeliverable;
...
function addDeliverableNotification(title) {
  ...
  addProgressMessage(
    `📄 New deliverable available: ${title}`,
    "success",
    `deliverable-${currentCampaignId}-${title}`
  );
...
function addReviewPrompt(campaign) {
  ...
  if (
    !container ||
    document.getElementById("reviewPrompt") ||
    document.getElementById("refinePrompt")
  )
    return;
  ...
  const message =
    campaign.pendingPhase === "final_signoff"
  ...
  const resumeLabel =
    campaign.pendingPhase === "final_signoff" ? "Finalize" : "Resume";

  prompt.innerHTML = `
                <div class="review-prompt-text">${message}</div>
                <div class="review-prompt-actions">
                    <button id="resumeBtnFlow" class="btn ${
                      resumeLabel === "Finalize" ? "btn-finalize" : ""
                    }">${resumeLabel}</button>
...
function showRefinePrompt(campaignId) {
  ...
  container.appendChild(prompt);
  // Add event listener to textarea to stop click propagation
  const refineTextArea = document.getElementById("refineText");
  if (refineTextArea) {
    refineTextArea.addEventListener("click", function (event) {
      event.stopPropagation();
    });
  }
  scrollToConversationBottom();
...
function newCampaign() {
  if (campaignInProgress || isCreatingCampaign) {
    const cancel = confirm(
      "A campaign is currently running. Click OK to cancel it and start a new one, or Cancel to keep waiting."
    );
    if (!cancel) {
